...
1. ~~**Milestone 0 from the spec (§13).** Measure how long the SSO cookie
   survives, using `probe_kmsi.py --reuse` over several days.~~ **Done
   2026-09-22**: persistent, 90-day absolute expiry (2026-12-20 14:32Z), with
   the value rotating on each renewal but the expiry standing still. §4 needed
   no revision. Still open: whether an idle timeout ends the session earlier —
   leave the jar untouched for 3 days, then a week, then stretch it.
...
3. **Watch a real parcel through delivery**, confirming that the status moves
   ...
   Every `unknown` is a row to add to `_STATUS_MAP`. Partially done: a live
   parcel moved `deliveryHandOver → inDistribution` and mapped correctly, so
   `IN_DISTRIBUTION` is now confirmed against real data. The remaining states
   are still unobserved.
4. ~~Confirm weight and dimensions~~ — **done 2026-09-21**. Kilograms and
   centimetres, against a real 5.85 kg / 80x53x32 parcel.
5. **Decide the terms-of-service question** (spec §12).
6. ~~Open a PR against `home-assistant/brands`~~ -- not applicable. Brands
   treats `custom_integrations/` as legacy since HA 2026.3.0; the icon ships
   in-repo at `custom_components/post_at/brand/`. Done.
